...
export const Court: React.FC<CourtProps> = ({ players, serving, teamName, rotationError }) => {
  return (
    <div className={`
        relative overflow-hidden p-4 rounded shadow-2xl
        ${rotationError ? 'ring-4 ring-red-500' : 'ring-1 ring-white/10'}
        bg-court-out
    `}>
      {/* Team Header inside Court Area */}
      <div className="flex justify-between items-center mb-3 px-2">
        <span className="font-black text-white uppercase italic tracking-wider text-lg drop-shadow-md">{teamName}</span>
        {serving && (
            <div className="flex items-center gap-2">
                <span className="w-3 h-3 rounded-full bg-yellow-400 animate-ping"></span>
                <span className="bg-yellow-400 text-black px-2 py-0.5 rounded text-[10px] font-black uppercase tracking-widest shadow-lg">Saque</span>
            </div>
        )}
      </div>

      {/* The Actual Court (Orange Area) */}
      <div className="bg-court-main border-4 border-white relative h-64 sm:h-80 shadow-[inset_0_0_20px_rgba(0,0,0,0.2)]">
            ...
            {POSITIONS_LAYOUT.map((layout) => {
               ...
               return (
                ...
                >
                  ...
                  <span className="absolute top-1 right-1 text-[9px] font-black text-black/20">{layout.pos}</span>

                  {playerByIndex ? (
                    <div className="flex flex-col items-center z-10 transform transition group-hover:scale-110">
                      <div className={`
                        w-10 h-10 sm:w-12 sm:h-12 rounded-full flex items-center justify-center 
                        font-black text-lg shadow-[0_4px_6px_rgba(0,0,0,0.3)] border-2 border-white
                        ${playerByIndex.name === 'Libero' ? 'bg-yellow-400 text-black' : 'bg-vnl-panel text-white'}
                      `}>
                        {playerByIndex.number}
                      </div>
                      <div className="mt-1 px-2 py-0.5 bg-black/60 backdrop-blur-sm rounded text-white text-[10px] font-bold uppercase tracking-wide truncate max-w-[80px]">
